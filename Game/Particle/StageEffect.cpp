...
const std::vector<D3D11_INPUT_ELEMENT_DESC> StageEffect::INPUT_LAYOUT =
{
...
void StageEffect::LoadShader()
{
	...
	// ピクセルシェーダ
	blob = DX::ReadData(L"Resources/Shaders/StageEffectPS.cso");
	DX::ThrowIfFailed(
		device->CreatePixelShader(blob.data(), blob.size(), nullptr, m_ps.ReleaseAndGetAddressOf())
	);

	// 頂点シェーダ
	blob = DX::ReadData(L"Resources/Shaders/StageEffectVS.cso");
	DX::ThrowIfFailed(
	...
	// ジオメトリシェーダ
	blob = DX::ReadData(L"Resources/Shaders/StageEffectGS.cso");
	DX::ThrowIfFailed(
